...
public static class ListOperator
{
    ...
    {
        return numbers
            .Where(number => number >= rangeFrom)
            .Where(number => number < rangeTo)
            .ToList();
    }
    ...
    {
        return numbers
            .ToDictionary(num => num, num => num * num);
    }
    ...
    public static Dictionary<int, int> CalculateFrequencyOfNumbers(List<int> numbers)
    {
        ...
        numbers
            .ForEach(num => frequency.Add(num, frequency.Remove(num, out int value) ? ++value : 1));
        return frequency;
    }
    ...
    {
        return values
            .Where(value => value.StartsWith(start) && value.EndsWith(end))
            .ToList();
    }
